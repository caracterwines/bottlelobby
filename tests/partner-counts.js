...
/* ── 7. The next pass is named, not forgotten ────────────────────── */
console.log('\n── the figures are true; the books they count are not yet one');
{
  /* Not a defect check — a marker. Every figure above is now an
     honest count of currentWinePortfolio, and the picker still offers
     a different list of a different length. The day the two become
     one book, this section fails and should be deleted, which is the
     only reliable way a deferred pass announces that it has landed. */
  const pool = w.eval('rPartnerWinesPool.length');
  const same = w.eval('JSON.stringify(rPartnerWinesPool) === JSON.stringify(tPartnerWinesPool)');
  if (pool === TOTAL) bad('picker and portfolio are the same size now — fold this section and the HANDOFF note');
  else ok('still two books: the picker offers ' + pool + ', the portfolio holds ' + TOTAL + ' (own pass, see HANDOFF)');
  if (!same) bad('the two picker pools have drifted apart — they were byte-identical copies');
  else ok('the two picker pools are still byte-identical, so neither buyer is reading a different one');
}
